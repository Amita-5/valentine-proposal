...
import { Heart, Music, Download, Volume2, VolumeX } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const toggleMusic = () => {
    if (!audioRef.current) {
      // Using a placeholder romantic royalty-free track
  ...
  const downloadPage = async () => {
    if (mainRef.current) {
      ...
      link.download = 'OurValentineProposal.png';
    ...
    >
      ...
      <div className="fixed top-6 right-6 z-50 flex flex-col gap-3">
        <button 
          ...
          className="p-3 bg-white/80 backdrop-blur-md rounded-full shadow-lg border border-pink-100 text-pink-500 hover:scale-110 transition-transform"
          title="Toggle Music"
        >
      ...
      {clickHearts.map(heart => (
        <div 
          key={heart.id}
          className="fixed pointer-events-none text-pink-500 transition-all duration-1000 ease-out z-[9999]"
          style={{ 
            left: heart.x, 
            top: heart.y,
